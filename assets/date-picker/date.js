function getWeekNumber(date) {
  ...
  class DatePicker extends HTMLElement {
    ...
    updateTodayStyle() {
        const todayElement = this.shadow.querySelector('.today');
        if (todayElement) {
          todayElement.classList.remove('today');
        }
    
        const todayButton = this.calendarDaysContainer.querySelector(`.month-day[data-date="${this.today.format('YYYY-MM-DD')}"]`);
        if (todayButton) {
    ...
    getSelectedDateFormatted() {
        const day = this.date.date.toString().padStart(2, '0');
        const month = this.date.monthNumber.toString().padStart(2, '0');
        const year = this.date.year.toString().slice(-2);
    ...
    render() {
        ...
        this.shadow.innerHTML = `
          ...
          <div class="calendar-dropdown ${this.visible ? 'visible' : ''} ${this.position}">
            ...
            <button type="button" class="today-btn" aria-label="Choose today's date">Обрати сьогоднішню дату</button>
          </div>
        `;
      
        const todayBtn = this.shadow.querySelector('.today-btn');
        todayBtn.addEventListener('click', () => this.chooseToday());
      }
